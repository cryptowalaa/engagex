...
import { Trophy, Target, Zap, Wallet, ArrowRight } from 'lucide-react'
  ...
  if (!publicKey) {
    return (
      <div className="min-h-screen bg-brand-dark">
        <Navbar />
        <div className="flex flex-col items-center justify-center min-h-[60vh] text-center gap-6 pt-16">
    ...
    <div className="min-h-screen bg-brand-dark">
      ...
      <main className="pt-24 pb-12 px-6 lg:px-8 max-w-7xl mx-auto">
        <div className="space-y-8">
          {/* FIXED: Welcome section with proper spacing and shortened address */}
          <div>
            <h1 className="text-3xl font-black mb-2 flex items-center gap-2">
              Welcome back 
              <span className="inline-flex items-center justify-center w-8 h-8 rounded-full bg-brand-green/20 text-brand-green text-lg">
                ◎
              </span>
            </h1>
            <p className="text-gray-400 font-mono text-sm">{shortenAddress(publicKey.toBase58())}</p>
          </div>

          {/* Stat cards */}
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {statCards.map(({ label, value, icon: Icon, color }) => (
              <div key={label} className="bg-brand-card border border-brand-border rounded-2xl p-5 hover:border-brand-green/30 transition-all">
          ...
          <div>
            <div className="flex items-center justify-between mb-4">
              ...
              <Link href="/missions" className="text-brand-green text-sm flex items-center gap-1 hover:underline">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {activeMissions.length === 0 ? (
                ...
                <Link key={mission.id} href={`/missions/${mission.id}`}
                  className="bg-brand-card border border-brand-border rounded-2xl p-5 hover:border-brand-green/30 transition-all block">
                  <div className="flex justify-between items-start mb-3">
                    <h3 className="font-bold text-white line-clamp-1">{mission.title}</h3>
                    <span className="text-brand-green font-bold text-sm whitespace-nowrap ml-2">{mission.reward_pool} {mission.currency}</span>
                  </div>
            ...
            <div className="bg-brand-card border border-brand-border rounded-2xl overflow-hidden">
              {rewards.length === 0 ? (
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {rewards.map(reward => (
                      <tr key={reward.id} className="border-b border-brand-border/50 hover:bg-white/5 transition-colors">
