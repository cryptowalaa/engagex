...
import { Clock, Users, Trophy, ExternalLink, Heart, MessageCircle, Share2, CheckCircle, TrendingUp, Link as LinkIcon, Medal } from 'lucide-react'
...
export default function MissionDetailPage() {
  ...
  async function loadData() {
    ...
    try {
      ...
      if (submissionIds.length > 0) {
        await loadComments(submissionIds)
      }
  ...
  async function loadComments(submissionIds: string[]) {
    try {
      ...
      if (error) {
        console.log('Comments table not found or error:', error)
        return
      }
  ...
  const submitComment = async () => {
    ...
    // Save comment to database if table exists
    try {
      ...
      console.log('Comment save error (table may not exist):', e)
    }
    ...
    // Reload comments
    await loadComments(submissions.map(s => s.id))
  }
