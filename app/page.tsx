...
export default function Home() {
  ...
  // FIXED: AvatarImage component - properly handles image loading with fallback
  const AvatarImage = ({ url, name }: { url: string | null, name: string | null }) => {
    const [error, setError] = useState(false)
    const [loaded, setLoaded] = useState(false)
    
    // If no URL or error, show fallback
    if (!url || error) {
      return (
    ...
    return (
      <div className="relative w-full h-full">
        ...
        {!loaded && (
          <div className="absolute inset-0 flex items-center justify-center bg-gradient-to-br from-brand-green/30 to-brand-purple/30 z-10">
            <span className="text-2xl md:text-3xl font-black text-white">
              {name?.[0]?.toUpperCase() || 'C'}
            </span>
          </div>
          ...
          {brandsLoading ? (
            ...
            <div className="flex gap-4 md:gap-6 overflow-x-auto pb-4 snap-x snap-mandatory scrollbar-hide items-start">
              {featuredBrands.map((featured) => (
                ...
                >
                  {/* FIXED: Brand logo container with explicit dimensions */}
                  <div className="w-16 h-16 md:w-24 md:h-24 rounded-2xl bg-gradient-to-br from-brand-purple to-brand-green mx-auto mb-3 md:mb-4 overflow-hidden">
