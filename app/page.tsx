...
export default function Home() {
  ...
  const AvatarImage = ({ url, name }: { url: string | null, name: string | null }) => {
    ...
    if (!url || error) {
      return (
        <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-brand-green/30 to-brand-purple/30">
  ...
  // FIXED: Brand Logo Component with proper image handling
  const BrandLogo = ({ url, name }: { url: string | null, name: string | null }) => {
    const [error, setError] = useState(false)
    const [loaded, setLoaded] = useState(false)
            ...
            <div className="flex gap-3 md:gap-4 overflow-x-auto pb-4 snap-x snap-mandatory scrollbar-hide">
              {topCreators.map((creator, index) => (
                ...
                >
                  ...
                  {/* FIXED: Avatar container with explicit dimensions */}
                  <div className="w-16 h-16 md:w-20 md:h-20 rounded-full bg-gradient-to-br from-brand-green to-brand-purple mx-auto mb-3 md:mb-4 overflow-hidden">
          ...
          {brandsLoading ? (
            ...
            <div className="flex gap-4 md:gap-6 overflow-x-auto pb-4 snap-x snap-mandatory scrollbar-hide items-start">
              {featuredBrands.map((featured) => (
                ...
                >
                  {/* FIXED: Brand logo container with BrandLogo component */}
                  <div className="w-16 h-16 md:w-24 md:h-24 rounded-2xl bg-gradient-to-br from-brand-purple to-brand-green mx-auto mb-3 md:mb-4 overflow-hidden flex items-center justify-center">
                    <BrandLogo url={featured.brand?.logo_url} name={featured.brand?.username} />
                  </div>
                  ...
                  {/* FIXED: Official badge styling */}
                  <div className="flex items-center justify-center gap-1 flex-wrap">
                    {featured.brand?.is_official_verified && (
                      <span className="inline-flex items-center gap-1.5 text-xs font-medium bg-[#FFAD1F]/15 text-[#FFAD1F] border border-[#FFAD1F]/30 px-2.5 py-1 rounded-full">
                        <Star size={10} className="fill-[#FFAD1F]" /> Official
                      </span>
                    )}
                    {featured.brand?.is_verified && !featured.brand?.is_official_verified && (
                      <span className="inline-flex items-center gap-1.5 text-xs font-medium bg-brand-green/15 text-brand-green border border-brand-green/30 px-2.5 py-1 rounded-full">
                        <CheckCircle size={10} className="fill-brand-green" /> Verified
                      </span>
                    )}
                  </div>
                </Link>
