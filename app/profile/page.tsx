...
export default function ProfilePage() {
  const { user, loading: userLoading, connected, refetch } = useUser()
  ...
  // ✅ FIXED: Check if wallet is connected but user not loaded
  if (!connected) {
    return (
      <div className="min-h-screen bg-brand-dark">
        <Navbar />
  ...
  // ✅ FIXED: If connected but no user data, show loading or error
  if (!user) {
    return (
      <div className="min-h-screen bg-brand-dark">
        <Navbar />
        <div className="flex pt-16">
          ...
          <main className="flex-1 p-6 lg:p-8">
            ...
            <div className="max-w-2xl bg-brand-card border border-brand-border rounded-2xl p-8 text-center">
              ...
              <button 
                onClick={refetch}
                className="px-4 py-2 bg-brand-green text-brand-dark rounded-lg"
              >
                Retry
              </button>
            </div>
