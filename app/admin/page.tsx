...
import { Shield, Users, Target, FileText, Trophy, Heart, MessageCircle, Share2 } from 'lucide-react'
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setPendingMissions(pm||[]); setUsers(us||[]);
      
      // Load submissions with engagements
  ...
  const saveEngagement = async (submissionId: string) => {
    try {
      // Update engagement - score will auto-calculate via trigger
      ...
      toast.success('Engagement updated! Score auto-calculated.')
      setEditingSubmission(null)
      await loadSubmissions() // Refresh to show new score
    } catch (e: any) {
  ...
  return (
    ...
    <main className="flex-1 p-6 lg:p-8 space-y-8">
      ...
      {/* NEW: Top Submissions with Engagement Management */}
      <div>
        <h2 className="text-xl font-bold mb-4 flex items-center gap-2"><Trophy size={20} className="text-yellow-400"/> Top Submissions <span className="text-xs text-gray-400 font-normal">(Auto-scored)</span></h2>
        <div className="bg-brand-card border border-brand-border rounded-2xl overflow-hidden">
          {submissions.length===0?<div className="p-8 text-center text-gray-500">No submissions yet</div>:(
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="border-b border-brand-border"><tr>{['Rank','Creator','Mission','Engagement','Score','Actions'].map(h=><th key={h} className="px-4 py-3 text-left text-xs text-gray-500 font-semibold uppercase">{h}</th>)}</tr></thead>
                <tbody>{submissions.map((sub, index)=>(
                  <tr key={sub.id} className="border-b border-brand-border/40 hover:bg-white/2">
                    <td className="px-4 py-3"><span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${index===0?'bg-yellow-500 text-black':index===1?'bg-gray-400 text-black':index===2?'bg-orange-500 text-black':'bg-brand-border text-gray-400'}`}>{index+1}</span></td>
                    <td className="px-4 py-3">
                      <p className="font-semibold text-white text-sm">{sub.creator?.username || shortenAddress(sub.creator?.wallet_address || '', 4)}</p>
                      <p className="text-xs text-gray-500">{sub.platform}</p>
                    </td>
                    <td className="px-4 py-3 text-gray-300 text-sm max-w-[150px] truncate">{sub.mission?.title}</td>
                    <td className="px-4 py-3">
                      {editingSubmission === sub.id ? (
                        <div className="flex gap-2 text-xs">
                          <input type="number" value={editForm.likes} onChange={e=>setEditForm({...editForm,likes:parseInt(e.target.value)||0})} className="w-14 bg-brand-dark border border-brand-border rounded px-2 py-1 text-white" placeholder="❤️"/>
                          <input type="number" value={editForm.comments} onChange={e=>setEditForm({...editForm,comments:parseInt(e.target.value)||0})} className="w-14 bg-brand-dark border border-brand-border rounded px-2 py-1 text-white" placeholder="💬"/>
                          <input type="number" value={editForm.shares} onChange={e=>setEditForm({...editForm,shares:parseInt(e.target.value)||0})} className="w-14 bg-brand-dark border border-brand-border rounded px-2 py-1 text-white" placeholder="🔄"/>
                        </div>
                      ) : (
                        <div className="flex gap-3 text-xs text-gray-400">
                          <span className="flex items-center gap-1"><Heart size={12} className="text-red-400"/>{sub.engagement?.likes||0}</span>
                          <span className="flex items-center gap-1"><MessageCircle size={12} className="text-blue-400"/>{sub.engagement?.comments||0}</span>
                          <span className="flex items-center gap-1"><Share2 size={12} className="text-green-400"/>{sub.engagement?.shares||0}</span>
                        </div>
                      )}
                    </td>
                    <td className="px-4 py-3"><span className="text-brand-green font-bold text-lg">{sub.score?.toFixed(0)||0}</span> <span className="text-xs text-gray-500">pts</span></td>
                    <td className="px-4 py-3">
                      {editingSubmission === sub.id ? (
                        <div className="flex gap-2">
                          <button onClick={()=>saveEngagement(sub.id)} className="text-xs bg-brand-green/10 text-brand-green border border-brand-green/20 px-3 py-1.5 rounded-lg font-semibold">💾 Save</button>
                          <button onClick={()=>setEditingSubmission(null)} className="text-xs bg-gray-500/10 text-gray-400 border border-gray-400/20 px-3 py-1.5 rounded-lg font-semibold">❌ Cancel</button>
                        </div>
                      ) : (
                        <div className="flex gap-2">
                          <button onClick={()=>startEdit(sub)} className="text-xs bg-blue-500/10 text-blue-400 border border-blue-400/20 px-3 py-1.5 rounded-lg font-semibold">✏️ Edit</button>
                          <button onClick={()=>deleteSubmission(sub.id)} className="text-xs bg-red-500/10 text-red-400 border border-red-400/20 px-3 py-1.5 rounded-lg font-semibold">🗑️ Delete</button>
                        </div>
                      )}
                    </td>
                  </tr>
