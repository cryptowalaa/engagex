...
export default function AdminRewards() {
  ...
  useEffect(() => {
    async function loadMissions() {
      try {
        ...
        // ✅ FIXED: Fetch all missions that are either:
        // 1. status = 'completed', 'expired', 'ended' OR
        // 2. status = 'active' BUT deadline has passed (actually expired)
  ...
  const distribute = async () => {
    ...
    try {
      // TODO: Implement actual Solana reward distribution
      // 1. Fetch submissions for this mission
      // 2. Calculate winners (max_winners limit)
      // 3. Calculate rewards: 60% creators, 20% engagers, 20% platform
      // 4. Execute USDC transfers on Solana
      // 5. Update mission status to 'rewarded'
      
      await new Promise(r => setTimeout(r, 1500))
      
      toast.success(`Rewards distributed for ${mission.title}! 🎉`)
      
      // Remove from list after distribution
          ...
          <div className="max-w-2xl bg-brand-card border border-brand-border rounded-2xl p-8 space-y-6">
            ...
            <div>
              ...
              {loading ? (
                ...
                >
                  ...
                  {missions.map(m => {
                    const isActuallyExpired = new Date(m.deadline) < new Date()
                    const displayStatus = isActuallyExpired && m.status === 'active' 
                      ? 'expired (was active)' 
                      : m.status
                    
                    return (
                      <option key={m.id} value={m.id}>
                        {m.title} ({m.reward_pool} {m.currency}) - {displayStatus}
                      </option>
                    )
                  })}
                </select>
              ...
              <div className="bg-brand-dark border border-brand-border rounded-xl p-4 space-y-2">
                <div className="flex justify-between text-sm">
                  ...
                  <span className={`font-medium ${
                    selectedMission.status === 'expired' ? 'text-red-400' : 
                    selectedMission.status === 'active' ? 'text-green-400' : 'text-blue-400'
                  }`}>
                    {selectedMission.status.toUpperCase()}
                  </span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-gray-400">Actual Status:</span>
                  <span className={`font-medium ${
                    new Date(selectedMission.deadline) < new Date() ? 'text-red-400' : 'text-green-400'
                  }`}>
                    {new Date(selectedMission.deadline) < new Date() ? 'EXPIRED' : 'ACTIVE'}
                  </span>
                </div>
                <div className="flex justify-between text-sm">
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span className="text-brand-green font-bold">
                    {selectedMission.reward_pool} {selectedMission.currency}
                  </span>
                </div>
              </div>
            )}
            ...
            {selected && (
              <div className="grid grid-cols-3 gap-4">
                <div className="bg-brand-dark border border-brand-border rounded-xl p-4 text-center">
                  <p className="text-2xl font-black text-brand-green">60%</p>
                  <p className="text-xs text-gray-500 mt-1">Creators</p>
                </div>
                <div className="bg-brand-dark border border-brand-border rounded-xl p-4 text-center">
                  <p className="text-2xl font-black text-brand-purple">20%</p>
                  <p className="text-xs text-gray-500 mt-1">Engagers</p>
                </div>
                <div className="bg-brand-dark border border-brand-border rounded-xl p-4 text-center">
                  <p className="text-2xl font-black text-yellow-400">20%</p>
