...
import { Trophy, Copy, CheckCircle, Wallet, ExternalLink, ChevronDown, ChevronUp } from 'lucide-react'
import { shortenAddress, formatUSDC } from '@/lib/utils/helpers'
import toast from 'react-hot-toast'

// Types from your existing database
interface Mission {
  id: string
  title: string
  reward_pool: number
...
interface LeaderboardEntry {
  ...
  shares: number
  rank: number
  status: string // pending, winner, paid
...
export default function AdminRewards() {
  ...
  // Load missions that need reward distribution
  useEffect(() => {
    loadMissions()
  }, [])
  ...
  async function loadMissions() {
    try {
      ...
      // ✅ Load missions that are: completed, expired, OR active but deadline passed
  ...
  // Load leaderboard when mission selected
  useEffect(() => {
  ...
  async function loadLeaderboard(missionId: string) {
    try {
      // ✅ Same logic as your existing mission leaderboard (page 97.tsx)
      ...
      // Transform to leaderboard format (same as your existing code)
      const entries: LeaderboardEntry[] = (subs || []).map((sub: any, index: number) => ({
        ...
        rank: index + 1,
        status: sub.status || 'pending', // pending, winner, paid
  ...
  // ✅ STEP 1: Select Top Winners (max_winners limit)
  async function selectWinners() {
    ...
    try {
      // Get top entries based on max_winners
      ...
      // Calculate reward per winner (60% of pool)
      const creatorPool = mission.reward_pool * 0.60
      const perWinner = creatorPool / topEntries.length

      // Update submission status to 'winner' and set reward_amount
  ...
  // ✅ STEP 2: Mark as Paid (manual payment system)
  async function markAsPaid(submissionId: string, creatorId: string, amount: number) {
    try {
      ...
      // 3. Update user's total_earned
      const { error: userError } = await supabase
        .from('users')
        .update({ 
          total_earned: supabase.rpc('increment_total_earned', { amount: amount })
        })
        .eq('id', creatorId)

      // Alternative if RPC not available:
      if (userError && userError.code === '42883') {
        // RPC not found, manual update
      ...
      <div className="flex pt-16">
        ...
        <main className="flex-1 p-6 lg:p-8">
          ...
          <div className="max-w-5xl space-y-6">
            ...
            {/* Leaderboard with Winners Selection */}
                    ...
                    <table className="w-full">
                      ...
                      <tbody>
                        {leaderboard.map((entry, index) => {
                          const isTopWinner = index < (selectedMission?.max_winners || 0)
